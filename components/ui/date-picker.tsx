...
export interface DatePickerProps extends Omit<React.ComponentProps<'input'>, 'type' | 'placeholder'> {
  placeholder?: string;
  onDateChange?: (date: string) => void;
}

const DatePicker = React.forwardRef<HTMLInputElement, DatePickerProps>(
  ({ className, placeholder, onDateChange, onChange, value, ...props }, ref) => {
    ...
    return (
      <div className="relative w-full">
        <input
          ...
          value={selectedDate}
          className={cn(
            'border-input ring-offset-background hover:border-input/80 disabled:hover:border-input flex h-10 w-full rounded border bg-(--color-background-color) px-4 py-2.5 pr-10 text-sm transition-all duration-200 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-clear-button]:hidden [&::-webkit-inner-spin-button]:hidden',
